...
class TextDataset(Dataset):
    def __init__(self, directory, tokenizer, model, label_idf="_age_", label_to_id=None, 
        ...
        self.file_paths = [os.path.join(directory, f) for f in os.listdir(directory) if os.path.isfile(os.path.join(directory, f)) and f.endswith('.jsonl')]
    ...
    def _load_in_data(self):
        ...
        for idx in tqdm(range(len(self.file_paths))):
            ...
            try:
                ...
                print(f"Malformed JSON at {file_path}")
                continue
            # print(text)

            if (isinstance(text["user_conversation"], list) and len(text["user_conversation"]) > 0):
                if isinstance(text["user_conversation"][0], dict):
                    parts = []
                    for turn in text["user_conversation"]:
                        if "text" in turn:
            ...
            if self.convert_to_llama2_format:
                messages_dict = []
                if (isinstance(text["user_conversation"][0], dict)):
                    for line in text["user_conversation"]:
                        if "text" in line:
                            if "### Human: " in line['text']:
                                messages_dict.append({'content': line['text'].replace("### Human:", "").strip() + " ", 'role': 'user'})
                            elif "### Assistant:" in line['text']: 
                                messages_dict.append({'content': line['text'].replace("### Assistant:", "").strip() + " ", 'role': 'assistant'})
                        elif "assistant" in line:
                            if "### Human: " in line['assistant']:
                                messages_dict.append({'content': line['assistant'].replace("### Human:", "").strip() + " ", 'role': 'user'})
                            elif "### Assistant:" in line['assistant']: 
                                messages_dict.append({'content': line['assistant'].replace("### Assistant:", "").strip() + " ", 'role': 'assistant'})
                else: 
                    for line in text["user_conversation"]:
                        if "### Human:" in line:
                            messages_dict.append({'content': line.replace("### Human:", "").strip() + " ", 'role': 'user'})
                        elif "### Assistant:" in line: 
                            messages_dict.append({'content': line.replace("### Assistant:", "").strip() + " ", 'role': 'assistant'})
                # if "### Human:" in usr_convo:
                #     user_msgs, ai_msgs = split_conversation(text, "### Human:", "### Assistant:")
                # elif "### User:" in usr_convo:
                ...
                try:
                    # print("\n".join(text["developer_instruction"]))
                    text = llama_v2_prompt(messages_dict, system_prompt="\n".join(text["developer_instruction"]))
            ...
            if self.new_format and self.if_remove_last_ai_response and self.include_inst:
                usr_convo = usr_convo[usr_convo.find("<s>") + len("<s>"):]
            elif self.new_format and self.include_inst:
                usr_convo = usr_convo[usr_convo.find("<s>") + len("<s>"):]
            elif self.new_format:
                usr_convo = usr_convo[usr_convo.find("<s>") + len("<s>"): usr_convo.rfind("[/INST]") - 1]
            
            label = file_path[file_path.rfind("_")+1:file_path.rfind(".jsonl")]
            # print(f"Extracted label: {label!r}, known labels: {list(self.label_to_id.keys())}")
